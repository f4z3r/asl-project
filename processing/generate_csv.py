    ...
    for operation in ["read", "write"]:
        ...
        for nclients in [1, 2, 4, 8, 16, 24, 32]:
            with open("processed/{}/{}/{}_clients.csv".format(basedir, operation, nclients), 'w') as csvfile:
                ...
                data = np.zeros(shape=(80, 2))

                for client in range(1, client_num + 1):
                    averages = np.zeros(shape=(80, 2))
...
def handle_memtier_mw(basedir, client_num):
    """Creates a csv file containing combined throughput, response time for every second."""
    os.mkdir("processed/{}".format(basedir))
    for operation in ["read", "write"]:
        for workers in [8, 16, 32, 64]:
            os.makedirs("processed/{}/{}/{}_workers".format(basedir, operation, workers))
            for nclients in [2, 4, 8, 14, 20, 26, 32]:
                with open("processed/{}/{}/{}_workers/{}_clients.csv".format(basedir, operation, workers, nclients), 'w') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(["Throughput", "latency(ms)"])
                    ...
                    for client in range(1, client_num + 1):
                        ...
                        for rep in [1, 2, 3]:
                            with open("preprocessed/{}/{}/{}_workers/{}_clients/client{}/rep{}.log".format(basedir, operation, workers, nclients, client, rep), 'r') as inputfile:
                                content = []
                                for line in inputfile.readlines():
                                    line_content = re.split(r"\s", line)
                                    line_content = list(filter(None, line_content))
...
def handle_middleware(basedir, mw_num):
    ...
    for operation in ["read", "write"]:
        for workers in [8, 16, 32, 64]:
            for nclients in [2, 4, 8, 14, 20, 26, 32]:
                with open("processed/{}/{}/{}_workers/mws_{}clients.csv".format(basedir, operation, workers, nclients), 'w') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(["Throughput", "Response time (ms)", "Queue time (ms)", "Queue length"])

                    data = np.zeros(shape=(80, 4))

                    for mw in range(1, mw_num + 1):
                        averages = np.zeros(shape=(80, 4))

                        for rep in [1, 2, 3]:
                            with open("preprocessed/{}/{}/{}_workers/{}_clients/mw{}/rep{}.log".format(basedir, operation, workers, nclients, mw, rep), 'r') as inputfile:
                                content = []
                                ...
                                for line in inputfile.readlines():
                                    line_content = re.split(r"\s", line)
                                    line_content = list(filter(None, line_content))
                                    content += [[int(line_content[4]), float(line_content[6]), float(line_content[7]), int(line_content[9])]]
...
if __name__ == "__main__":
    if sys.argv[1] == "all":
        ...
        handle_memtier_mw("bench_1mw", 1)
        handle_memtier_mw("bench_2mw", 2)
        handle_middleware("bench_1mw", 1)
        handle_middleware("bench_2mw", 2)
